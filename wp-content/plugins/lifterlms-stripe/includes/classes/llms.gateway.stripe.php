...
class LLMS_Payment_Gateway_Stripe extends LLMS_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->title = __( 'Credit or Debit Card', 'lifterlms-stripe' );
		...
		// add admin l10n
		add_filter( 'lifterlms_js_l10n_admin', array( $this, 'admin_l10n' ) );

	}

	/**
	 * localize js error messages/warnings on the admin panel
	 * @param    array     $strings  existing strings
	 * @return   array
	 * @since    4.3.0
	 * @version  4.3.0
	 */
	public function admin_l10n( $strings ) {

		$strings['This looks like a Publishable Key! Your Secret Key should go here.'] = esc_html__( 'This looks like a Publishable Key! Your Secret Key should go here.', 'lifterlms-stripe' );
		$strings['This looks like a Secret Key! Your Publishable Key should go here.'] = esc_html__( 'This looks like a Secret Key! Your Publishable Key should go here.', 'lifterlms-stripe' );
	...
	/**
	 * Execute a charge
	 * @param  object   $order   instance of the LLMS_Order to execute a charge for
	...
	public function process_refund( $transaction, $amount = 0, $note = '' ) {
		...
		if ( $ret->is_error() ) {
			...
			$this->log( $ret, 'Stripe `process_refund()` finished with errors' );
		...
		// check for success
			...
			if ( isset( $res->status ) && 'succeeded' === $res->status ) {
				...
				$this->log( $res, 'Stripe `process_refund()` finished' );
				return $res->id;
	...
	public function settings_fields( $fields, $gateway_id ) {
		...
		$fields[] = array(
			'type'  => 'custom-html',
			'value' => '
				<h4 id="llms_gateway_stripe_live_heading">' . __( 'Stripe Live API Credentials', 'lifterlms-stripe' ) . '</h4>
		...
		$fields[] = array(
			'type'  => 'custom-html',
			'value' => '
				<h4 id="llms_gateway_stripe_test_heading">' . __( 'Stripe Test API Credentials', 'lifterlms-stripe' ) . '</h4>
				<p>' . sprintf( __( 'Enter your Stripe API credentials to process transactions via Stripe. Learn how to access your Stripe API Credentials <a href="%s">here</a>.', 'lifterlms-stripe' ), 'https://support.stripe.com/questions/where-do-i-find-my-api-keys' ) . '</p>
			',
		);
