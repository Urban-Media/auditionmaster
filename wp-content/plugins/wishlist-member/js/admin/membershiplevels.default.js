...
//			$.post('<?php echo admin_url()?>', form.serialize(), function(res) {
			$.post(ajaxurl, form.serialize(), function(res) {
				if ( $.trim(res) == '' ) {
					...
					if (  jQuery('.updated_mlevel').length ) { jQuery('.updated_mlevel').parent().remove(); }
					jQuery('.wrap').find('p').after('<div class="updated fade"><p class="updated_mlevel">Membership Levels Updated</p></div>');
					jQuery('.updated').fadeOut(6000);
				} else {

					if (  jQuery('.updated_mlevel').length ) { jQuery('.updated_mlevel').parent().remove(); }
					jQuery('.wrap').find('p').after('<div class="error fade"><p class="updated_mlevel">' +$.trim(res) +'</p></div>');
					jQuery('.updated').fadeOut(6000);
				}
				self.cancel(id);
			});
		...
		self.remove = function(id) {
			...
			$.post(ajaxurl, data, function(res) {
				...
				$("#clonefrom option[value=" +id +"]").remove(); //remove level from clone option
			});
